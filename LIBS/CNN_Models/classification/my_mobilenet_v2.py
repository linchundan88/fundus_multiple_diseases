...
https://github.com/vvigilante/mobilenet_v2_keras/blob/master/mobilenet_v2_keras.py
"""
...
from keras.activations import relu as relu6
# Mobile, Float16, Relu6  Avoid big output value. but we use float32 so
...
def MobileNetv2(input_shape, num_classes, width_multiplier=1.0):
    """MobileNetv2
    ...
    x = Conv2D(num_classes, (1, 1), padding='same', name='logits', use_bias=True)(x)

    x = Activation('softmax', name='softmax')(x)
    output = Reshape((num_classes,), name='out')(x)
...
def MobileNetv2_448(input_shape, num_classes, width_multiplier=1.4):
    """MobileNetv2
    This function defines a MobileNetv2 architectures.
    # Arguments
        input_shape: An integer or tuple/list of 3 integers, shape
    ...
    x = _inverted_residual_block(x, roundup(int(96*width_multiplier)), (3, 3), t=6, strides=1, n=3)
    #following add
    x = _inverted_residual_block(x, roundup(int(128*width_multiplier)), (3, 3), t=6, strides=2, n=3)
...
def MobileNetv2_112(input_shape, num_classes, width_multiplier=1.0):
    """MobileNetv2
    This function defines a MobileNetv2 architectures.
    # Arguments
        input_shape: An integer or tuple/list of 3 integers, shape
    ...
    x = _inverted_residual_block(x, roundup(int(16*width_multiplier)), (3, 3), t=1, strides=1, n=1)
    x = _inverted_residual_block(x, roundup(int(24*width_multiplier)), (3, 3), t=6, strides=2, n=2)
    x = _inverted_residual_block(x, roundup(int(32*width_multiplier)), (3, 3), t=6, strides=1, n=3)
    x = _inverted_residual_block(x, roundup(int(64*width_multiplier)+fix), (3, 3), t=6, strides=2, n=4)
    x = _inverted_residual_block(x, roundup(int(96*width_multiplier)), (3, 3), t=6, strides=1, n=3)
    x = _inverted_residual_block(x, roundup(int(160*width_multiplier)), (3, 3), t=6, strides=2, n=3)
    x = _inverted_residual_block(x, roundup(int(320*width_multiplier)), (3, 3), t=6, strides=1, n=1)
...
if __name__ == '__main__':
    # model1 = MobileNetv2(input_shape=(224, 224, 3), num_classes=100)
    # model1 = MobileNetv2_112(input_shape=(112, 112, 3), num_classes=100)
    model1 = MobileNetv2_448(input_shape=(448, 448, 3), num_classes=100)
