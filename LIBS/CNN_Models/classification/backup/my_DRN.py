...
def _residual_block(block_function, filters, repetitions, is_first_layer=False, block_seq=0):
    ...
    def f(input):
        for i in range(repetitions):
            ...
            input = block_function(filters=filters, init_strides=init_strides,
                    is_first_block_of_first_layer=(is_first_layer and i == 0), block_seq=block_seq)(input)
        return input
...
def basic_block(filters, init_strides=(1, 1), is_first_block_of_first_layer=False, block_seq=0):
    ...
    """
    def f(input):
        #4 blocks, block 0 an 1 are the same as resnet, block 2 and block3 use dilated convolution
        if block_seq == 2:
        ...
        # second conv block dilation_rate?
...
class DRN_A_Builder(object):
    ...
    def build(input_shape,  num_outputs, block_fn, repetitions, init_filters=64,
        ...
        for i, r in enumerate(repetitions):
            block = _residual_block(block_fn, filters=filters, repetitions=r,
                                    is_first_layer=(i == 0), block_seq=i)(block)
            filters *= 2
    ...
    @staticmethod
    def build_DRN_A_18(input_shape, num_outputs):
        return DRN_A_Builder.build(input_shape, num_outputs, basic_block, [2, 2, 2, 2],
                                   init_filters=64, init_kernal_size=7)

    @staticmethod
    def build_DRN_A_34(input_shape, num_outputs):
        return DRN_A_Builder.build(input_shape, num_outputs, basic_block, [3, 4, 6, 3],
                                   init_filters=64, init_kernal_size=7)

    @staticmethod
    def build_DRN_A_50(input_shape, num_outputs):
        return DRN_A_Builder.build(input_shape, num_outputs, bottleneck, [3, 4, 6, 3],
                                   init_filters=64, init_kernal_size=7)

    @staticmethod
    def build_DRN_A_101(input_shape, num_outputs):
        return DRN_A_Builder.build(input_shape, num_outputs, bottleneck, [3, 4, 23, 3])
...
if __name__ == '__main__':
    ...
    #params:21,307,650
    model1 = DRN_A_Builder.build_DRN_A_18(input_shape=(224, 224, 3), num_outputs=2)
