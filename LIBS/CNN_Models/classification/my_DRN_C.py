...
def basic_block(filters, is_first_layer=False, block_seq=0):
    ...
    def f(input):
        if block_seq == 0:
            if is_first_layer:  #after first conv bn relu
            ...
            conv2 = _bn_relu_conv(filters=filters, kernel_size=(3, 3))(conv1)

        if block_seq in [1, 2, 3]:
            if is_first_layer:
            ...
            conv2 = _bn_relu_conv(filters=filters, kernel_size=(3, 3),
                                  dilation_rate=(4, 4))(conv1)

        return _shortcut(input, conv2)

    return f

#DRN-C the last two blocks in order to remove Gridding artifacts
...
def bottleneck(filters, is_first_layer=False, block_seq=0):
    ...
    def f(input):
        ...
        if block_seq == 0:
            if is_first_layer:   #DRN-C  just after the first_conv_bn_relu
                conv_1 = Conv2D(filters=filters, kernel_size=(1, 1),
                                  strides=(1, 1),
                                  padding="same",
                                  kernel_initializer="he_normal",
                                  kernel_regularizer=l2(1e-4))(input)
            else:
                conv_1 = _bn_relu_conv(filters=filters, kernel_size=(1, 1))(input)

            conv_2 = _bn_relu_conv(filters=filters, kernel_size=(3, 3))(conv_1)
            conv_3 = _bn_relu_conv(filters=filters * 4, kernel_size=(1, 1))(conv_2)

        if block_seq in [1, 2, 3]:
            if is_first_layer:
                conv_1 = _bn_relu_conv(filters=filters, kernel_size=(1, 1), strides=(2, 2))(input)
            else:
                conv_1 = _bn_relu_conv(filters=filters, kernel_size=(1, 1))(input)
            ...
            if is_first_layer:
                conv_1 = _bn_relu_conv(filters=filters, kernel_size=(1, 1),
                                            dilation_rate=(1, 1))(input)
            else:
                conv_1 = _bn_relu_conv(filters=filters, kernel_size=(1, 1),
                                         dilation_rate=(2, 2))(input)

            conv_2 = _bn_relu_conv(filters=filters, kernel_size=(3, 3), dilation_rate=(2, 2))(conv_1)
            conv_3 = _bn_relu_conv(filters=filters * 4, kernel_size=(1, 1), dilation_rate=(2, 2))(conv_2)
            ...
            conv_2 = _bn_relu_conv(filters=filters, kernel_size=(3, 3), dilation_rate=(4, 4))(conv_1)
            conv_3 = _bn_relu_conv(filters=filters * 4, kernel_size=(1, 1), dilation_rate=(4, 4))(conv_2)

        return _shortcut(input, conv_3)
