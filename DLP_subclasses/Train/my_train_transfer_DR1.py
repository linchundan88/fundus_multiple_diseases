...
def train_task(filename_csv_train, filename_csv_valid,
               epoch_header=2, epoch_fine_tuning=20):
    ...
    weight_class_start = np.array([1, 8])  #23648, 2020
    weight_class_end = np.array([1, 8])
    balance_ratio = 0.93
    ...
    my_gen = My_images_weight_generator(files=train_files, labels=train_labels,
                                        ...
                                        num_class=NUM_CLASSES, img_aug_mode=IMG_AUG_ROTATE_MODE, batch_size=BATCH_SIZE_TRAIN, image_shape=image_shape)
    #endregion
    ...
    if GPU_NUM > 1:
        print('convert train top model to Multiple GPU...')
        model_train_top = ModelMGPU(model_train_top, GPU_NUM)
        print('convert train top model to Multiple GPU OK')
...
train_task(filename_csv_train=filename_csv_train, filename_csv_valid=filename_csv_valid,
           epoch_header=3, epoch_fine_tuning=20)
